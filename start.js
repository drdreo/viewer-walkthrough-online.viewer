...
app.get("/api/forge/oauth", function (req, res) {
    Axios({
        ...
        .then(function (response) {
            ...
            console.log(`Successfully authenticated! Token expires in: ${response.data.expires_in / 60}`);
            console.log(`...redirecting to create a bucket`);
...
app.get("/api/forge/datamanagement/bucket/create", function (req, res) {
    ...
    Axios({
        ...
        .then(function (response) {
            console.log("Successfully created bucket!", response.data);
            res.redirect("/api/forge/datamanagement/bucket/detail");
        })
        .catch(function (error) {
            console.log(error.response.data.reason);
            if (error.response && error.response.status === 409) {
                console.log(`...redirecting to bucket details`);
                res.redirect("/api/forge/datamanagement/bucket/detail");
                return;
            }
            // Failed
...
app.get("/api/forge/datamanagement/bucket/detail", function (req, res) {
    Axios({
        ...
        .then(function (response) {
            // Success
            console.log("Successfully loaded bucket:", response.data);
...
app.get("/api/forge/modelderivative/:urn", function (req, res) {
    ...
    Axios({
        ...
        .then(function (response) {
            // Success
            console.log("Successfully forged", response.data);
...
app.get("/download/:urn", function (req, res) {
    const urn = req.params.urn;
    ...
    Axios({
        ...
        .then(function (response) {
            // Success
            console.log("Successfully downloaded", response.data);
            const urns = [];
            response.data.derivatives.forEach(derivative => {
                derivative.children.forEach(child => {
                    console.log(child);
                    if (child.urn && child.urn.includes(".svf")) {
                        urns.push(child.urn);
                    } else if (child.children) {
                        child.children.forEach(c => {
                            if (c.urn && c.urn.includes(".svf")) {
                                urns.push(c.urn);
                            }
                        });
                    }
            ...
            res.redirect("/api/download-file/" + urn + "/" + encodeURIComponent(urns[0]));
        })
        .catch(function (error) {
            // Failed
            console.log(error);
...
// Route /api/forge/modelderivative
app.get("/api/download-file/:urn/:derivativeUrn", function (req, res) {
    const urn = req.params.urn;
